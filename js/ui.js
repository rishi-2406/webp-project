...
const UICtrl = (function() {
    ...
    return {
        ...
        updateIncomeInsights: function(incomeData) {
            document.getElementById('salary-amount').textContent = formatCurrency(incomeData.salary);
            document.getElementById('bonus-amount').textContent = formatCurrency(incomeData.bonus);
            document.getElementById('investment-amount').textContent = formatCurrency(incomeData.investment);
            document.getElementById('other-income-amount').textContent = formatCurrency(incomeData.other);
            
            const totalIncome = incomeData.salary + incomeData.bonus + incomeData.investment + incomeData.other;
            
            // Calculate and update percentages
            if (totalIncome > 0) {
                document.getElementById('salary-percentage').textContent = Math.round((incomeData.salary / totalIncome) * 100) + '%';
                document.getElementById('bonus-percentage').textContent = Math.round((incomeData.bonus / totalIncome) * 100) + '%';
                document.getElementById('investment-percentage').textContent = Math.round((incomeData.investment / totalIncome) * 100) + '%';
                document.getElementById('other-income-percentage').textContent = Math.round((incomeData.other / totalIncome) * 100) + '%';
            }
        },
        ...
        populateRecentTransactions: function(transactions) {
            ...
            recentTransactions.forEach(transaction => {
                ...
                const iconName = transaction.type === 'income' ? 'arrow-down' : 'arrow-up';
        ...
        initMiniCharts: function(incomeData, expenseData) {
            // Income mini chart
            const incomeCtx = document.getElementById('income-mini-chart').getContext('2d');
            new Chart(incomeCtx, {
                type: 'line',
                data: {
            ...
            // Expense mini chart
            const expenseCtx = document.getElementById('expense-mini-chart').getContext('2d');
        ...
        setCurrentMonth: function() {
            const now = new Date();
            document.getElementById('month-selector').value = now.getMonth().toString();
        }
    };
})();
